...
% \citet{Mula2006,Diaz-Madronero2014}
% Review
...
A review of old models can be found in the second part of \citet{Candace1995} which look at the models where uncertainty is placed on the yields (\eg ordered quantity may differ from received quantity) and where the others parameters may depend on these randomness.
More recently, we can cite the reviews in \citet{Mula2006} and \citet{Diaz-Madronero2014}.
One of the most complete review for the post 2000 publications dealing with stochastic lot-sizing can be found in \citet{Aloulou2014}.
...
Stochastic production planning also deals with huge size problems like electricity production (see \citet{Rozenknop2013}) but it leads to develop special heuristics to solve them.



Since we consider uncertainty, demand may be unsatisfied.
In order to control backorder, many methods exists.
An old and classic one consists in using backorder costs (see \citet{Zangwill1969,Absi2009}) that is paid for undelivered quantities.
However, as explained in \citet{Tempelmeier2013}, backorder costs may be hard to quantify due to indirect consequences of undelivered quantities that is why he recall several service level measures that can be used as a substitute.
Chance constraints, which are quite close to $\gamma$-service level, are also used (see \citet{Tarim2004,Gicquel2018}).
Unsatisfied demand is also controlled using robust optimization like in \citet{Gyulai2017} or in \citet{Minoux2018} who obtains better results than classic stochastic optimization when targeting very low risk levels.
