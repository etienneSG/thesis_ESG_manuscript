...
\esgil{Revoir la formulation. Insister sur le fait que le nombre de setup match parce que on n'a pas de scheduling.}
...
The production and the inventory of item $i$ must satisfy a demand $\demand_t^i$ at the end of period $t$. When production of item $i$ is not used to satisfy the demand, it can be stored but incurs a unit holding cost $\holding_t^i>0$ per period. For each item $i$, there is an initial inventory $s_0^i\in\RR_+$.
...
\esgil{Dernier paragraphe redondant avec l'introduction ?}
