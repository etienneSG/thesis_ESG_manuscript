...
Our heuristic consists in solving (2SA-$m$)\vlil{préciser que cela signifie tirer $m$ nouveaux scénario connaissant le passé} as defined in Problem~\eqref{eq:Uniform-CLSP-BS-2SA-m:backorder} at the beginning of each period $t$.
...
For lack of time\vl{Due to time constraints}, we have not implemented $K$-mean algorithm.
...
By looking at the values of the backorder cost, we can see that the values computed by method proposed in \cref{sec:PDP:numerical-experiments:drawing-realization-of-demand} are very small especially for small desired service level like 85\%.\vl{unclear}
Thus, the method to compute the backorder\vl{cost ?} is too optimistic and must be improved.
