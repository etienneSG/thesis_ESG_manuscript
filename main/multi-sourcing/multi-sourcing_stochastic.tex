...
Note that risk is often concentrate\vl{?} to single-item problem.
...
On the other hand, probabilistic constraint may be hard to linearize since it may not be convex.\vl{thus requires additional binary variables}
...
Robust constraint\vl{, in its vanilla form,} consists in satisfying the constraint in every possible outcome.
...
\vlil{il faudrait rappeler que l'AVAR constraint est strictement plus dure que la chance constraint - pour un même niveau.}
