...
As for production planning decisions, uncertainty may come from efficiency of machine (and affect time needed for the production of one item), capacities (due to breakdown, strike) or any other part of the supply chain like the procurement of raw materials.
...
It goes from the more conservative which does not allow stock-out to methods controlling stock-out probability, stock-out quantity or other relevant indicators.
...
Second, worst cases must be not ``not too bad''.\vl{2 negations}
Multi-sourcing decisions are long-term decisions and cannot be easily changed.
However, when bad outcomes occur, it should be controlled and should remained reasonable since it will impact the company competitiveness for long periods.
...
Several models are possible and will be discussed in \cref{sec:multi-sourcing:stochastic:model:discussion}.
...
We introduce a parameter $\servicelvl\in\sqbracket{0,1}$ which controls the part of demand delivered on time.
...
The goal is to satisfy the service level measure\vl{constraint} at minimum cost.
...
In order to measure and control the service level, we use the \emph{Average Value at Risk} which is a \emph{coherent measure of risk}\vl{coherent risk measure} widely studied by Rockafellar and Uryasev (see for example \cite{Rockafellar2000,Rockafellar2002}) and chose the definition given in \cite{Follmer2004}.\vl{cite Delbaen et al}
...
\vlil{Je ne comprends pas les deux flèches}
...
\vlil{$p$ undefined in \eqref{eq:stoch-multi-sourcing:measurability}}
...
\vlil{il faut définir ce que l'on entends par ces différentes contraintes, et noter que l'on ne considère qu'un cas simple de "robust constraint", qui est d'ailleurs un cas extreme de AVAR et Proba constraint}
...
\esgil{Show to Frederic and Vincent before writing it.}
...
This approximation is a \emph{two-stage approximation} as we distinguish between two levels of information over the uncertainty: affectation decisions are the \emph{first stage} variables, while all other decisions are \emph{second stage} variables.
...
% \begin{algorithm}[H]
%   \SetLine
%   \KwIn{MIP~\eqref{eq:stoch-multi-sourcing:linearized}}
%   \KwOut{Feasible multi-sourcing}
%   \KwParameters{$N$, $\tau$}
%   \KwResult{}
...
%   }
%   }
%   \caption{Heuristic to solve MIP~\eqref{eq:stoch-multi-sourcing:linearized}}
