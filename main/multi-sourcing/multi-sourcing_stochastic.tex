...
% \citet{Brian2005}
% => two stage: assignment then production (single period)
% => risk neutral and risk averse

% \citet{Garavelli2003}
% => use simulation to quantify impact of flexibility configuration (multiple product, multiple plants, multiple market)


% \citet{Meena2011}
% => each supplier has different rate of failure, capacity but demand is equaly reparted
% => risk on disruption (one supplier which do not deliver)
% => one item, one period
...
% \citet{Arikan2017}
% => news-vendor with $\AVaR$
% Many references on news-vendors and risk aversion \citet{Vipul2000,Choi2008}

% \citet{Ahmed2007}
% => coherent measure of risk for inventory problem (single item) for both single and multi-period
% => objective minimize a risk averse function (like $\AVaR$) instead of a risk neutral function (like expectation)
% => production decisions no multi-sourcing
...
\begin{subequations}\label{eq:stoch-multi-sourcing:bender}
  \begin{align+}
    ...
    % \\
    % & \ds u_{\omega}\in \cW && \forall\omega\in\scenarios,
