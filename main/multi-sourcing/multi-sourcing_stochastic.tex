...
For the inventory, $\AVaR_{1-\servicelvl}\bracket{\va\inventory}$ looks like the safety stock in the case of a fill rate service level $\servicelvl$.
...
In order to linearize $\AVaR$ constraint \eqref{eq:stoch-multi-sourcing:AVaR}, we introduce for each period $t$ and each item $i$ auxiliary variables $\mu_t^i\in\RR$ and $\bracket{\alpha_{t,\omega}^i}_{\omega}\in\RR_+^{\scenarios}$.
...
\begin{algorithm}[!htb]
\Input{Sets $\REF$, $\plants$ and $\scenarios$, parameters $\affect_p^i$, $\rate_{pt}^i$, $\capacity_{pt}$, $\lb_{pt}^i$, $\ub_{pt}^i$ and $\servicelvl$, and demand $\demand_{t,\omega}^i$}
\Output{Assignment $(\open_p^i)_{p,i}$}
\Parameters{$N$, $\tau$}
\BlankLine
\lFor{$p\in\plants$ \And $i\in\REF$}
{
  $\open_p^i:=1$ \tcp*[f]{current assignment}
}
$cost:=+\infty$ \tcp*[r]{cost of current assignment}
\BlankLine
\lFor{$t\in\range{\horizon}$ \And $i\in\REF$}
{
  $\hat{\demand}_t^i:=\frac{1}{\card{\scenarios}}\sum_{\omega\in\Omega}\demand_{t,\omega}^i$ \tcp*[f]{average demand}
}
$W:=\sum_{i\in\REF}\sum_{t=1}^{\horizon}\rate^i\hat{\demand}_t^i$. \tcp*[r]{cumulative sum of working hours}
$\capacity:=\sum_{p\in\plants}\sum_{t=1}^{\horizon}\capacity_{pt}$. \tcp*[r]{cumulative capacity of plants}
\BlankLine
\If{$\capacity<W$}
{
...
\BlankLine
\BinarySearch{\em $\alpha\in\sqbracket{1,\frac{\capacity}{W}}$ with $N$ iterations}
{
  \lFor{$t\in\range{\horizon}$ \And $i\in\REF$}
  {
    $\demand_t^i:=\alpha\hat{\demand}_t^i$ \tcp*[f]{demand in Problem~\eqref{eq:det-multi-sourcing}}
  }
  Solve Problem~\eqref{eq:det-multi-sourcing} with a time limit $\tau$\;
  \eIf{\em Problem~\eqref{eq:det-multi-sourcing} is infeasible}
  {
    Decrease $\alpha$\;
  }
  {
    ...
    \eIf{\em Program~\eqref{eq:stoch-multi-sourcing:linearized} with assignment $(\tilde{\open}_p^i)_{p,i}$ is infeasible}
    {
      Increase $\alpha$\;
    }
    {
      \If{$new\_cost \le cost$}
      {
...
A binary search algorithm on $\alpha$ is justified by the following proposition.
...
We did not try to implement this method, but it might be a promising way to speed up the resolution of formulation~\eqref{eq:stoch-multi-sourcing:linearized}.
...
We propose seven criteria to compare the constraints.
...
Thus, we easily see that the $\AVaR$ constraint enables to control the proportion of bad cases and their value.
