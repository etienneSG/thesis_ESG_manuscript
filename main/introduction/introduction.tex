...
It was funded by Argon Consulting which is an independent consulting firm whose mission is to help its clients improve every part of their Supply Chain (from the acquisition of raw materials to the delivery of final products) and conducted throughout an industrial partnership with the \'Ecole des Ponts ParisTech.
...
Specifically, we study continuous and integer number of setups in deterministic and stochastic settings.
...
As in \cref{sec:intro:en:multi-sourcing}, our stochastic lot-sizing problem is also solved by first doing a two-stage approximation and then a sampling of scenarios in order to get a mixed integer linear program.
...
The experimentations made in \cref{chap:PDP:numerical-experiments} already prove that the company that provides the datasets can reduce its inventory costs while keeping a good ability to satisfy the demand.
...
In this case, the company aims at deciding an assignment which maximize the demand that can be satisfied.
