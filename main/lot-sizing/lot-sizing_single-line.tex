...
\esgil{ajouter que c'est temps continu et un des premier modèles}

\esgil{Find article from Gayon}
...
\vlil{Multiple extensions of this model have been studied. A first consists in considering time varying demand, like in \cite{Resh1976,Donaldson1977,Barbosa1978} where they show polynomial time algorithm for demand of the form $\demand(t)=\alpha t+\gamma$ or $\demand(t)=\alpha t^{\beta}$. }
...
However, in general, finding the optimal solution is challenging \esgil{voir dans l'article si NP-hard}. See for example \cite{Padmanabhan1990} where the demand is assumed to be a function of the inventory and they proposed a method from numerical analysis to find the optimal solution.
\vl{they propose a numerical method to find the optimal solution} where a numerical method must be used to find the optimal solution.
Another extension considers a time varying holding cost due to inflation (see \cite{Vrat1990}) for which they also propose a method from numerical analysis to find the optimal solution.


Although they are deterministic, many models allow lost sales like \cite{Salameh2003} which introduces a shortage cost or \cite{Park1982} where a prefixed percentage of unsatisfied demand is lost while the remaining is backordered.


\medskip


The models we just presented all deal with a single item problems.
In many real case, an assembly line can produce more than one item.
A first multi-item model was proposed by \cite{Hadley1963} which also note that perhaps the most important real world constraint is budget restriction on the amount that can be invested in inventory.
...
\esgil{ajouter des commentaires sur la méthode et le résultat. \cite{Ohno2001} étudie un modèle où il y a des contraintes sur l'inventaire pour éviter qu'il n'explose...}


\esgil{Clarifier l'origine de la distinction continuous revious vs. periodic review.}
As explained in \cite{Eynan2007}, when many items are produced on a single line, using periodic review (\ie cover-sizes) instead of continuous review (\ie \vl{la virgule est étrange} lot-sizes) may ease the coordination between the production of various items.
Similar approach was also studied by \cite{Madigan1968} and more recently \cite{Chiu2014} propose models with a kind of scheduling constraints between periodic productions of items.
Finally, \cite{Bomberger1966} and \cite{Goyal1974} impose that every replenishment cycles (cover-sizes) be a multiple of a unit cover-size (which can be a variable in the optimization process like in \cite{Silver1976}).
\esgil{In real applications, it corresponds to mutaliser la commande}
...
\esgil{Mettre en intro qu'on regarde plutôt la commande.}
...
\esgil{Préciser que c'est plutôt de l'ordering}

One of the first stochastic ordering problems is the news-vendor problem \cite{Edgeworth88, Arrow1951}.
...
Many deterministic continuous review inventory models have been extended to a stochastic setting.
\cite{Candace1995} and more recently \cite{Ziukov2015} propose a complete review of the extensions of EOQ, EPQ or Joint Economic Lot Sizing models.
\esgil{Vérifier joint et référence à la section précédente}
...
\esgil{Déorouler naturellement le paragraphe puis dire plus tard que par ailleurs, il existe une litterature fuzzy}

\esgil{Changer placer l'aléa en modeliser l'alea}
...
\cite{Sana2011} proposes extensions of the news-vendor problem adding an uncertainty on sales price.
Some of these extensions allow sales price to be correlated demand.


Finally, \cite{Gallego1998} shows how to adapt ($r$,$q$) policies to \tbc\ taking into account random demand during lead-time.
\esgil{modifier voir si partially known.}
Its results do not depend from distribution and are not too far from the optimum if distribution was known during the optimization process.


\medskip


As in deterministic cases, the models presented above consider only a single item.
\cite{Daeschner1975} considers a multi-item inventory system with stochastic demands and with a coupling budget constraint.
Moreover, the ability production depend on viability on 
...
Let us ... there exist another trend to study stochastic continuous namely fuzzy set theory...
\tbc
...
Among other models, we can cite \cite{Hsieh2002} which introduced two fuzzy production inventory models with fuzzy parameters: the first with crisp production quantity (\ie a production in a set in the classical sense) and the second with production quantity in a fuzzy set.
Contrary to the majority of the literature, \cite{Lee1999} and \cite{Wang2007} propose a model for the economic order quantity in a fuzzy framework but without backorder.
