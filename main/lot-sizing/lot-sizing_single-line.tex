...
\vlil{Multiple extension of this model has been studied. A first consits in considering time varying demand, like in \cite{Resh1976,Donaldson1977,Barbosa1978} where they show polynomial time algorithm
for demand of the form $\demand(t)=\alpha t+\gamma$ or $\demand(t)=\alpha t^{\beta}$. }
...
However, in general, finding the optimal solution is difficult like in \cite{Padmanabhan1990} where demand is a function of the inventory and \vl{they propose a numerical method to find the optimal solution} where a numerical method must be used to find the optimal solution.
Another \vl{extension consider} variation relies on a time varying holding cost due to inflation (see \cite{Vrat1990}) which also need numerical method to find the optimal solution.
...
As explained in \cite{Eynan2007}, when many items are produced on a single line, using periodic review (\ie cover-sizes) instead of continuous review (\ie \vl{la virgule est étrange} lot-sizes) may make coordination between item productions easier.
...
Its results do not pendent\vl{?} from distribution and are not too far from the real optimum if distribution was considered during the optimization process.
